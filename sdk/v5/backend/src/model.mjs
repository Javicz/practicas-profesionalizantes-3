...
export function getUserById(db, userId) {
    return getQuery(db, 'SELECT id, username, password_hash FROM user WHERE id = ?', [userId]);
}
...
export function userHasPermission(db, userId, endpointPath) {
    var sql = `
    ...
    `;
    return getQuery(db, sql, [userId, endpointPath]).then(function(row) {
        return row.total > 0;
    });
}